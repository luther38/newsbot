...
class TwitchAPI():
    ...
    def auth(self) -> TwitchAuth:
        ...
        if res.status_code != 200:
            ...
            o = TwitchAuth(access_token=token['access_token'], 
    ...
    def getUser(self, auth: TwitchAuth, username: str) -> None:
        ...
        if res.status_code != 200:
            logger.error(f"Failed to get user information. StatusCode: {res.status_code}, Error: {res.text}")
        else:
    ...
    def searchForUser(self, auth: TwitchAuth, username: str = '') -> None:
        if username == '':
            logger.error(f"Request to search for user was requested but no user was given.")
        else:
            uri: str = f"{self.apiUri}/search/channels?query={username}"
            header = self.__header__(auth)
            res = get(uri, headers=header)
            if res.status_code != 200:
                logger.error(f"Attempted to pull user information but failed. status_code: {res.status_code}, output: {res.text}")
            else:
                l = list()
                j = loads(res.content)
                for i in j['data']:
    ...
    def getGame(self, auth: TwitchAuth, game_id: int) -> TwitchGameData:
        ...
        if res.status_code != 200:
            logger.error(f"Attempted to get Twich Game data but failed on game_id: {game_id}. output: {res.text}")
    ...
    def getVideos(self, auth: TwitchAuth, id: int = 0, user_id: int = 0, game_id: int = 0) -> List[TwitchVideo]:
    ...
    def getClips(self, auth: TwitchAuth, user_id: int = 0, clip_id: str = '', game_id: int= 0) -> List[TwitchClip]:
        ...
        if user_id != 0:
            ...
            logger.error(f"Clips was requested but was given invalid parameters, returning empty object.")
            return ""

        res = get(uri, headers=self.__header__(auth))
        clips = list()
        if res.status_code != 200:
            logger.error(f"Clips request returnd a bad status_code. Code: {res.status_code}, Error: {res.text}")
            return clips
        else:
            json = loads(res.content)
            for i in json['data']:
